    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combined with context, automatically invokes tools to organize entities and relationships from the knowledge graph, initializes simulation individuals, and gives them unique behaviors and memories based on the reality seed
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow rate, recommendation algorithm, active time period for each individual, speech frequency, event trigger and other parameters based on simulation requirements and reality seed
          </p>
              ...
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Platform 1: Square / Information Feed</span>
                  </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation environment preparation completed. Ready to start running simulation.</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish automatically planned <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time, where each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes elapsed</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First run? Strongly recommend switching to Custom mode to reduce rounds — quickly preview results and reduce error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
// Log deduplication: record key info from last output
...
watch(currentStage, (newStage) => {
  if (newStage === 'GenerateAgentPersona' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter configuration generation phase, start polling
    if (!configTimer) {
...
// Calculate auto-generated rounds from configuration (do not use hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Configuration not yet generated
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Configuration data incomplete
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Ensure max rounds is at least 40 (recommended minimum) to avoid slider range issues
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User set custom rounds, pass max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation with custom rounds: ${customMaxRounds.value}`)
  } else {
    // Use auto-generated rounds, do not pass max_rounds parameter
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Read ${res.data.expected_entities_count} entities from Neo4j graph`)
...
const loadPreparedData = async () => {
  ...
  // Get configuration (use real-time endpoint)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Configuration not yet ready — polling...')
        startConfigPolling()
      }
    }
  } catch (err) {
