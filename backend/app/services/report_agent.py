...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            logger.info(
                f"Section {section.title} did not include 'Final Answer:'; using LLM output as final content (tool calls: {tool_calls_count})"
            )
        ...
        if response is None:
            logger.error(f"Section {section.title} forced conclusion returned empty response")
    ...
    def generate_report(
        ...
        try:
            ...
            # Initialize structured log logger (agent_log.jsonl)
            ...
            # Phase 2: Sequentially generate sections (save per section)
    ...
    def chat(
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # execute at most 1 tool call
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        distinct from the agent_log.jsonl structured log.

        Args:
            report_id: Report ID
            from_line: Line number to start reading from (for incremental fetch; 0 = from the beginning)
    ...
    def save_section(
        ...
        Call immediately after each section is generated to enable per-section streaming output.
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove duplicate Markdown title lines at the start of content that match the section title
        2. Convert ### and deeper heading levels to bold text
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it's a duplicate of the section title (skip if duplicate within first 5 lines)
                if i < 5:
                ...
                # because section titles are added by the system — content should have no headings
        ...
        while cleaned_lines and cleaned_lines[0].strip() in ['---', '***', '___']:
            cleaned_lines.pop(0)
            # Also remove any empty lines after the separator
            while cleaned_lines and cleaned_lines[0].strip() == '':
                cleaned_lines.pop(0)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assembles the complete report from saved section files and cleans up heading issues.
        """
        folder = cls._get_report_folder(report_id)
        ...
        # Sequentially read all section files
        sections = cls.get_generated_sections(report_id)
        for section_info in sections:
            md_content += section_info["content"]
        
        # Post-processing: clean heading issues across entire report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report main title (#) and section titles (##), convert other levels (###, #### etc.) to bold
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check for duplicate title (same title appearing within the last 5 lines of content)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate title and subsequent empty lines
                    i += 1
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section title incorrectly using #, corrected to ##
                        processed_lines.append(f"## {title}")
                        prev_was_heading = True
                    else:
                        # Other first-level titles convert to bold
                        processed_lines.append(f"**{title}**")
